...
def filter_image_name(path):
    ...
    for ext in ext_list:
        if file_name.endswith(ext):
            ...
            break
    # new_path should have old path but with file name changed
...
def upload_sketch(_INPUT_=_INPUT_, ID=ID):
    ...
    cv2_imwrite(room_path + filtered_name + '.origin.png', origin)
    ...
    cv2_imwrite(room_path + filtered_name + '.sketch.png', sketch)
    return ID + '_' + ID
...
def request_result(_INPUT_=_INPUT_, _COLOR_IMAGE_=_COLOR_IMAGE_, ID=ID):
    ...
    smoothed_careless, flat_careless, blended_smoothed_careless, blended_flat_careless = refine_image(careless, sketch,
                                                                                                      origin)
    # ID = datetime.datetime.now().strftime('H%HM%MS%S')
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.smoothed_careless.png', smoothed_careless)
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.flat_careless.png', flat_careless)
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.blended_smoothed_careless.png', blended_smoothed_careless)
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.blended_flat_careless.png', blended_flat_careless)
    ...
    smoothed_careful, flat_careful, blended_smoothed_careful, blended_flat_careful = refine_image(careful, sketch,
                                                                                                  origin)
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.smoothed_careful.png', smoothed_careful)
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.flat_careful.png', flat_careful)
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.blended_smoothed_careful.png', blended_smoothed_careful)
    cv2_imwrite(room_path + '/' + filtered_name + ID + '.blended_flat_careful.png', blended_flat_careful)
...
def main():
    ...
    for file in input_files:
        if not file.endswith(tuple(ext)):
            ...
            room_path = request_result(_INPUT_=file)
    # copy all png files from room_path to _OUTPUT_
